...
err_t page_not_found_handler(struct netconn* connection_context){
  netconn_write(connection_context, HTTP_NOT_FOUND END_OF_HEADER, strlen(HTTP_NOT_FOUND)+strlen(END_OF_HEADER),NETCONN_NOCOPY);
  static const char not_found_page[] = "<html><body><p> Page Not found</p></body></html>";
  netconn_write(connection_context, not_found_page, sizeof(not_found_page) ,NETCONN_NOCOPY);
  return ERR_OK;
}

err_t alive_handler(struct netconn* connection_context){
  netconn_write(connection_context, HTTP_OK CONTENT_TYPE_JSON END_OF_HEADER, strlen(HTTP_OK)+strlen(CONTENT_TYPE_JSON)+strlen(END_OF_HEADER),NETCONN_NOCOPY);
...
static int32_t match_endpoint(const char** endpoint_repository, uint32_t repository_size, const char* request_endpoint){
  for(uint32_t endpointID = 0; endpointID < repository_size;endpointID++){
    if(strlen(endpoint_repository[endpointID]) != 0 && strncmp(endpoint_repository[endpointID], request_endpoint, strlen(endpoint_repository[endpointID])) == 0){
      return endpointID;
    }
  }
...
static err_t handle_request(struct netconn* connection_context,char* request){
  ...
  if(strncmp(get,request, strlen(get)) == 0){
    int32_t endpointID = match_endpoint(GET_ENDPOINTS, MAX_GET_ENDPOINTS, request + strlen(get));
...
bool register_endpoint(HTTP_Method method, const char* endpoint_match_string,endpoint_handler handler){
  switch(method){
    #if MAX_GET_ENDPOINTS > 0
    case GET:
    {
